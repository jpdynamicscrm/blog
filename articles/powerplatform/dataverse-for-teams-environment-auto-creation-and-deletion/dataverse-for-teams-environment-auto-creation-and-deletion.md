...
本記事では Dataverse for Teams 環境の概要と、自動作成・削除の仕組みについて、以下の Q1〜Q6 の質問に回答する形でご説明させていただきます。
...
### この記事でわかること
- Dataverse for Teams 環境とは何か、通常の Dataverse との違い
- 追加のライセンスなしで使える条件と、Teams の外では使えない制約
...
## 目次
- [Dataverse for Teams 環境とは](#about)
...
<h2 id="about">Dataverse for Teams 環境とは</h2>

Dataverse for Teams は、Microsoft Teams の中に組み込まれたデータ管理の仕組みです。
2020 年 9 月に導入されました。

ふだん Teams を使っている方が、プログラミングの知識がなくても、Teams の中でかんたんにアプリやチャットボット、自動処理（フロー）を作れるようにするための土台となる機能です。
作ったアプリのデータ（たとえば申請の一覧や在庫の情報など）は、この Dataverse for Teams 環境の中に保存されます。

**通常の Dataverse との違い**

Microsoft には「Dataverse」という、より高機能なデータ管理の仕組みもあります。
Dataverse for Teams は、この Dataverse をベースにしていますが、Teams の中で手軽に使えるように機能をしぼった版です。
主な違いは以下の通りです。

| 項目 | Dataverse for Teams | Dataverse（通常版） |
|---|---|---|
| 使える場所 | Teams の中だけ | Teams の内外どちらでも |
| 追加ライセンス | 不要（対象の Microsoft 365 に含まれる） | Power Apps / Power Automate 等のライセンスが必要 |
| 環境ごとの容量 | 2 GB | テナントの容量に依存 |
| 環境の作り方 | Teams 内の操作で自動作成 | Power Platform 管理センターから作成 |
| 外部からの API アクセス | 不可 | 可能 |
| ゲストの操作 | アプリの実行のみ（作成・編集は不可） | ロールに応じて操作可能 |

Dataverse for Teams は、EDU A1 プランと SUB SKU を除く、Microsoft 365 の多くのプランに含まれています。
つまり、すでに Microsoft 365 をお使いの場合は、追加の費用なしで利用できます。

ただし、Teams の外でアプリを使いたい場合や、外部システムから直接データにアクセスしたい場合は、環境を通常の Dataverse にアップグレードする必要があります。
アップグレードには、Power Apps や Power Automate などのライセンスが別途必要です。

**チームと環境の関係**

Dataverse for Teams 環境は、Teams のチームと 1 対 1 で紐づきます。
1 つのチームに対して作られる環境は 1 つだけで、チームのメンバー全員がその環境のアプリやデータを共有します。

チームの所有者には「システム管理者」の権限が自動で割り当てられ、チームのメンバーにはデータの閲覧や編集ができる「Teams メンバー」の権限が割り当てられます。

Power Platform 管理センターの環境一覧では、種類が「Microsoft Teams」と表示されている環境が Dataverse for Teams 環境です。

＜参考資料＞
- [Teams 環境の Microsoft Dataverse について (Docs)](https://learn.microsoft.com/ja-jp/power-platform/admin/about-teams-environment)
- [Dataverse for Teams 概要 (Docs)](https://learn.microsoft.com/ja-jp/powerapps/teams/overview-data-platform)
- [Power Platform ライセンス FAQ - Dataverse for Teams (Docs)](https://learn.microsoft.com/ja-jp/power-platform/admin/powerapps-flow-licensing-faq#dataverse-for-teams)
