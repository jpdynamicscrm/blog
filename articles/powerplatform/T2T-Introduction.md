...
なお、__キャンバス アプリと Power Automate については原則として移行前の事前準備が必要__ です。
対応いただかなかった場合、移行後にコンポーネントが消滅する場合もございますので、ご注意ください。
...
技術サポートでは、[弊社技術サポート営業時間](https://learn.microsoft.com/ja-jp/power-platform/admin/support-overview#what-hours-are-considered-local-business-hours-for-support)内でのご支援を承っております。
そのため夜間や休日に移行を実施する場合には、営業時間内での技術サポートからの完了報告が叶いませんので、翌営業日の営業時間に実施結果をご報告いたします。
...
テナント移行の自動ロールバックは行われませんので、ユーザー マッピングのみが完了していない状態となります。
