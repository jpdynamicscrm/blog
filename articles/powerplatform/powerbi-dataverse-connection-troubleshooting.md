...
クライアント端末から Dataverse endpoint へ TCP 接続できるかどうかを、Windows PowerShell を使って確認します。

#### 手順 1. PowerShell を開く

Windows のスタート メニューから「PowerShell」と検索し、**Windows PowerShell** を開きます。

#### 手順 2. ポートの疎通を確認する

以下のコマンドを実行します。`<environment>` の部分は、お使いの Dataverse 環境の名前に置き換えてください。
...
実行結果に表示される **TcpTestSucceeded** の値を確認します。

- **True** の場合: TCP レベルでの疎通は問題ありません。
- **False** の場合: クライアント端末からそのポートへの通信がブロックされている可能性があります。

#### 手順 3. 名前解決と IP アドレス直接指定での接続を確認する

あわせて、DNS の名前解決結果と、解決された IP アドレスへの直接接続もご確認いただくと、切り分けがしやすくなります。
...
上記で表示された IP アドレスを `<RemoteAddress>` に置き換えて、以下を実行します。
...
FQDN では False だが IP アドレス直接指定では True になる場合 (またはその逆) は、DNS や名前解決に関連する問題の可能性があります。
