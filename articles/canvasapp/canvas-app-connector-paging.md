---
title: Power Apps キャンバス アプリで nextLink を使って続きのデータを取得する方法
...
本記事では、Power Apps キャンバス アプリからコネクタを呼び出したときに一部のデータしか取得できない場合に、応答の `nextLink` と `$skiptoken` を使って続きのページを取得する方法をご紹介します。
...
- [3. nextLink を使用して続きのページを取得する](#3-nextLink-を使用して続きのページを取得する)
  ...
  - [3-2. ページ数が決まっていない場合](#3-2-ページ数が決まっていない場合)
...
Power Apps キャンバス アプリから一覧を取得するコネクタ アクションを実行すると、対象データが存在していても、一部のレコードしか返らない場合があります。
...
本記事では、次のように `nextLink` を使って続きのページを取得し、1 つのコレクションへ追加します。
...
抽出したトークンを `$skiptoken` に指定すると、続きのページを取得できます。

<a id='3-2-ページ数が決まっていない場合'></a>

### 3-2. ページ数が決まっていない場合

取得するページ数が事前に分からない場合は、`nextLink` が空になるまで要求を繰り返す必要があります。

> [!CAUTION]
> `ForAll()` はレコードの処理順を保証せず、可能な場合は並列に処理します。各ページの要求が直前のページから取得した `$skiptoken` に依存するため、`Sequence()` と `ForAll()` を組み合わせても安全な逐次処理にはなりません。

[ForAll 関数](https://learn.microsoft.com/ja-jp/power-platform/power-fx/reference/function-forall)

ページ数が決まっていない場合は、Power Automate で `nextLink` が空になるまで順番に処理する方法をご検討ください。取得対象が 2 ページ以内であれば、次のようにキャンバス アプリ内で 2 回の要求を明示できます。
...
どちらかが常に適しているわけではありません。取得件数、呼び出し頻度、保守方法、応答時間を考慮して選択します。
...
途中のページ取得に失敗した場合、結果用コレクションには一部のデータだけが残る可能性があります。実運用では `IfError()` などを使用し、取得失敗を利用者へ通知する処理を追加してください。
...
取得するページ数が決まっている場合は `With()` や `Set()` で要求を明示し、ページ数が決まっていない場合は Power Automate の利用もご検討ください。
