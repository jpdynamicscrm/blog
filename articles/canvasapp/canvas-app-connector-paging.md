...
また、Dataverse や SharePoint リストなどをデータ ソースとして直接接続する場合は、本記事のようなコネクタ アクションのページングではなく、**委任 (デリゲーション)** によってサーバー側で絞り込みや並べ替えを行える場合があります。委任に対応した関数と数式を使用すると必要なレコードだけを取得できるため、そもそも全件取得が不要になることもあります。データ ソースの選択肢がある構成では、委任もあわせてご検討ください。

[キャンバス アプリの委任について - Power Platform サポート ブログ](https://jpdynamicscrm.github.io/blog/canvasapp/Canvas-app-delegation/)
