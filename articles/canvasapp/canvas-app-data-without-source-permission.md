...
- [4. 採用前に確認しておきたい注意点](#4-採用前に確認しておきたい注意点)
  ...
  - [4-2. 委任が効かない](#4-2-委任が効かない)
  - [4-3. 監査・運用継続性](#4-3-監査・運用継続性)
- [補足](#5-補足)
...
フローの呼び出しは、例えば `OnStart` などアプリの起動時に実行する形が考えられます。
以下は、3 列のデータを受け取ってコレクションに格納するサンプルです。

![OnStart でクラウドフローを呼び出してコレクションに格納](./canvas-app-data-without-source-permission/3-2-app-onstart-formula.png)
...
フロー経由で取得したデータはアプリ側でコレクションとして扱われるため、Power Apps の **委任機能は利用できません**。
コレクションの行数上限に収まる範囲のデータ量でご利用ください。

[キャンバス アプリの委任について - Microsoft Learn](https://learn.microsoft.com/ja-jp/power-apps/maker/canvas-apps/delegation-overview)

<a id='4-3-監査・運用継続性'></a>

### 4-3. 監査・運用継続性

データソースへのアクセスがフロー作成者の権限で行われるため、以下の点にご留意ください。

- データソース側の監査ログでは「誰が参照したか」を追跡できない
- フロー作成者のアカウントが無効化されると、アプリ全体のデータ取得が停止する可能性がある
