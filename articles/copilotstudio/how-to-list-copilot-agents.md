...
Copilot Studio にて作成されたエージェントは Dataverse または Dataverse for Teams に保存されます。本記事では Dataverse および Dataverse for Teams からエージェントを一覧を取得する方法と Power Platform 管理センターで用意されたレポートでエージェントを一覧を確認する方法をご案内いたします。
