...
本記事は上記公開情報の制限内容を補足・説明してものとなります。
...
タイムアウトの制限は緩和できず、延ばすことはできないものとなっております。
...
###  送信同期要求
フロー内の同期処理の待ち時間となります。
例えばアクションを通じて、リクエスト投げたら2分以内に応答がないと引っかかる
INSERTやGET要求などAPIごとに制限対象

例えば、Dataverse テーブルやSQLへの書き込み時などに注意が必要

<a id='anchor-outbound-asynchronous-request'></a>

### 送信非同期要求
フロー内の非同期処理の待ち時間となります。
実行しているアクションでの時間

例えば、承認
承認リクエストを投げて、応答を待つ場合など

<a id='anchor-inbound-request'></a>

### 受信要求
応答アクションがフロー内で使用されている場合、受信要求120秒の制限Vが適用される場合がございます。<br>
